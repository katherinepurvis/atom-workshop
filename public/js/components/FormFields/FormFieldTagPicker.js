    ...
    return (
      <div className="form__field__suggestions">
        {this.state.suggestions.map((suggestion) => {
          ...
          return (
            <a className="form__field__suggestion" key={suggestion.id} title={suggestion.id} onClick={updateFn}>{suggestion.webTitle} ({suggestion.type}) <span className="form__field__suggestion__id">{suggestion.id}</span></a>
          );
        })}
      </div>
    );
